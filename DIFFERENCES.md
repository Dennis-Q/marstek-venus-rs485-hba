...
| **Solar-aware strategy** | New strategy (`hba_strategy_solar_aware`) that decides each control cycle whether to export solar to the grid (Zero import) or absorb it locally (Self-consumption), based on two conditions: cheap hours remain today AND remaining solar forecast exceeds a configurable threshold. Designed as the Dynamic v2 default sub-strategy for households that charge during cheap/negative hours and want to maximize export revenue during the pre-cheap solar window. Cheap-hour detection is today-only, so tomorrow's prices published mid-afternoon don't incorrectly keep the battery discharging all evening. Falls back silently to Self-consumption when prices or the solar forecast entity are unavailable. HBC has no equivalent. |
